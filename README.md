...
| 사용자별 로그인·로그아웃 | ✅ | 최초 1회 + 조건부 반복 | utmp, wtmp, btmp, lastlog 변경을 auditd가 기록하도록 설정한다. |
| 접속 터미널 ID와 위치 | 🟡 | 최초 1회 + 조건부 반복 | 세션 파일과 SSH `LogLevel VERBOSE`로 TTY·원격 IP를 확보할 수 있다. 물리 위치는 VPN/NAC/CMDB 연계가 필요하다. |
| 서버 접근 성공 및 실패 | 🟡 | 최초 1회 + 조건부 반복 | SSH 상세 로그와 로그인 세션·실패 파일은 설정한다. 중앙 상관분석과 서비스별 접근 감사는 없다. |
| 데이터 및 다른 자원 접근 성공·실패 | 🟡 | 최초 1회 + 조건부 반복 | 선택한 경로 변경과 일반 사용자의 `EACCES/EPERM` 파일 접근 실패를 기록한다. 업무 데이터·DB·앱 감사 대상은 미정이다. |
| 시스템·커널·부팅 이벤트 | ✅ | 최초 1회 + 조건부 반복 | journald를 영구 저장하고 rsyslog로 전달하도록 설정한다. |
| 주요 시스템 및 네트워크 설정파일 | ✅ | 최초 1회 + 조건부 반복 | 계정, sudo, SSH, PAM, hosts, resolv, sysctl 등 존재하는 경로를 audit watch로 설정한다. |
| cron 실행 및 패키지 업데이트 이력 | 🟡 | 최초 1회 + 조건부 반복 | cron 설정파일 변경은 감사하지만 cron의 실제 실행과 yum/dnf/apt/dpkg 이력 규칙은 없다. |
| 사용자 명령어 실행 이력 | ✅ | 최초 1회 + 조건부 반복 | 로그인 사용자의 `auid`를 기준으로 `execve`를 감사한다. 명령 인자의 민감정보 보호정책은 추가로 필요하다. |
| 로그 1년 이상 보관 | 🟡 | 최초 1회 + 조건부 반복 | journald에 365일을 설정했지만 4GB 제한으로 조기 삭제될 수 있다. audit/logrotate 및 중앙 저장소 보존정책이 필요하다. |
...
| 관리자 권한 파일·서비스 사용 시도 | 🟡 | 최초 1회 + 조건부 반복 | sudoers 변경과 일반 접근 거부는 일부 기록한다. sudo/su/systemctl/SUID 실행 감사 규칙은 미구현이다. |
| 비인가 파일·자원·서비스 사용 시도 | 🟡 | 최초 1회 + 조건부 반복 | 파일 접근 실패는 기록한다. 승인 파일·포트·서비스 목록 및 실시간 비교·알림은 없다. |
| 행위별 성공·실패 구분과 유형별 보존 | 🟡 | 최초 1회 + 조건부 반복 | 접근 실패 errno 규칙은 있다. 전체 이벤트의 성공·실패 규칙과 로그 유형별 보존기간은 미정이다. |
...
| 정책 항목 | 상태 | 실행 성격 | Ansible 적용 가능 범위 또는 미구현 사유 |
|---|---:|---|---|
| 1인 1계정 발급 | 🚫 | 조건부 반복 | 계정 생성·중복 UID 점검은 자동화할 수 있지만 실제 사용자와 계정의 1:1 관계는 HR/IAM·발급 절차가 보장해야 한다. |
| 패스워드는 사용자 본인이 관리 | 🚫 | 조건부 반복 | Ansible은 비밀번호 자체가 아닌 복잡도·만료·이력 정책만 관리해야 한다. |
| 2종류 10자 또는 3종류 8자, 권고 12~64자 | ⚪ | 최초 1회 + 조건부 반복 | `pam_pwquality`로 구현 가능하지만 조건부 OR를 배포판 공통으로 정확히 표현하기 어려워 최소 12자·2종류의 단일 기준을 설계했다. 아직 코드화하지 않았다. |
| Null 패스워드 금지 | ⚪ | 최초 1회 + 조건부 반복 | `PermitEmptyPasswords no`와 `/etc/shadow` 점검으로 구현 가능하나 미구현이다. |
| 사용자 ID와 동일한 패스워드 금지 | ⚪ | 최초 1회 + 조건부 반복 | `pam_pwquality usercheck`로 구현 가능하나 구형 OS 지원 확인과 Role 작성이 필요하다. |
| 예측 가능한 패스워드 금지 | ⚪ | 최초 1회 + 조건부 반복 | 사전검사, 반복·연속문자 제한은 가능하다. 조직명·연도 등 모든 패턴의 완전한 차단은 별도 사전/IAM이 필요하다. |
| 주기성 패스워드 재사용 금지 | ⚪ | 최초 1회 + 조건부 반복 | 동일한 최근 비밀번호는 `pam_pwhistory`로 차단 가능하다. 숫자·계절명 변형 패턴은 표준 PAM만으로 완전히 막기 어렵다. |
| 패스워드 힌트 금지 | 🚫 | 최초 1회 + 조건부 반복 | 일반 Linux 로컬계정의 표준 기능이 아니며 포털·메일·헬프데스크 절차에서 통제해야 한다. |
| 초기 패스워드 최초 접속 시 변경 | ⚪ | 조건부 반복 | 계정 발급 후 `chage -d 0`으로 강제 가능하지만 사용자 목록과 발급 흐름이 정해지지 않았다. |
| 로그인 실패 10회 이하·로그·연결 해제 | ⚪ | 최초 1회 + 조건부 반복 | `pam_faillock`, SSH `MaxAuthTries`, audit/PAM 로그로 가능하다. OS별 PAM 방식과 잠금시간이 확정되지 않았다. |
| 비밀번호 6개월 만료, MFA 사용자는 1년 | ⚪ | 최초 1회 + 조건부 반복 | `chage -M 180/365`로 가능하다. 사람·서비스 계정 구분과 MFA 대상 원천정보가 필요하다. |
| 직전 2개 비밀번호 재사용 금지 | ⚪ | 최초 1회 + 조건부 반복 | `pam_pwhistory remember=2`로 가능하지만 OS별 PAM Role이 아직 없다. |
| 일방향 암호화 저장 | ⚪ | 최초 1회 + 조건부 반복 | OS별 SHA-512 또는 yescrypt 정책 적용이 가능하다. 기존 해시는 다음 비밀번호 변경 때 전환된다. 아직 코드화하지 않았다. |
