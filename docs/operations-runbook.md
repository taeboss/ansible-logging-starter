...
U0207은 `/etc/hosts` 내용을 변경하지 않고 소유자와 권한만 조정한다. 심볼릭
링크이면 링크가 가리키는 원본 일반 파일을 조치한다. `0600` 권한은 일반 사용자와
서비스 계정의 파일 접근을 제한하므로 파일럿 서버에서 이름 해석 영향을 먼저 확인한다.
...
실제 적용 후 업무·서비스 계정으로 필요한 호스트 이름을 조회한다. 상세 기준과
검증 명령은 `docs/remediation/U0207.md`에서 확인한다.

U0507은 SSH 서비스가 실행 중인 서버에서 현재 설치된 OpenSSH 관련 패키지만
공식 저장소의 최신 버전으로 업데이트한다. 현재 관리 세션 단절을 방지하기 위해
SSH 서비스 재시작과 서버 재부팅은 수행하지 않는다.
...
실제 적용 후 별도의 새 터미널에서 신규 SSH 접속을 확인하고, 콘솔 또는 대체
접속 수단을 확보한 유지보수 시간에 필요한 SSH 서비스 재시작을 직접 수행한다.
상세 기준은 `docs/remediation/U0507.md`에서 확인한다.

U0509는 현재 설치된 glibc 관련 패키지만 공식 저장소의 최신 버전으로
업데이트한다. 전체 OS 업그레이드, GCC 설치·업데이트, 서비스 재시작과 서버
재부팅은 수행하지 않는다.
...
실제 적용 후 출력되는 패키지·실행 버전과 재시작 필요 여부를 검토하고, 업무
영향도를 확인한 뒤 필요한 서비스 재시작 또는 서버 재부팅을 별도 수행한다.
상세 기준은 `docs/remediation/U0509.md`에서 확인한다.

U0510은 현재 설치된 OpenSSL 관련 패키지만 공식 저장소의 최신 버전으로
...
상세 기준은 `docs/remediation/U0510.md`에서 확인한다.
